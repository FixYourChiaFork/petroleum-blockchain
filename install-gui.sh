...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the petroleum python virtual environment."
  echo "Execute '. ./activate' before running."
	exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The Petroleum Blockchain GUI can not be installed or run by the root user."
	exit 1
fi
...
echo ""
echo "Petroleum blockchain install-gui.sh completed."
echo ""
