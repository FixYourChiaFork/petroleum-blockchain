...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    all_targets = []
    stop_searching_for_farmer = "xpt_target_address" not in config["farmer"]
    stop_searching_for_pool = "xpt_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xpt_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xpt_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "xpt_target_address" not in config["farmer"]:
        print(
            f"Setting the petroleum destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["xpt_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["xpt_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xpt_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xpt_target_address" not in config["pool"]:
        print(f"Setting the petroleum destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["xpt_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["xpt_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xpt_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the PETROLEUM destination addresses, edit the `xpt_target_address` entries in"
