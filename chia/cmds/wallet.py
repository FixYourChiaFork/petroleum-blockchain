...
@wallet_cmd.command("send", short_help="Send Petroleum to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@wallet_cmd.command("send_from", short_help="Transfer all Petroleum away from a specific puzzle hash")
@click.option(
    "-p",
    "--rpc-port",
    help=(
...
@click.option(
    "--launcher-hash",
    help="Set the launcher hash, you should get it from your Petroleum wallet",
    type=str,
    default=None,
)
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
