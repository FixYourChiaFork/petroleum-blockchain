...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "xpt"), encode_puzzle_hash(new_ph_2, "xpt"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "xpt"))
            ...
            assert config["farmer"]["xpt_target_address"] == encode_puzzle_hash(new_ph, "xpt")
            assert config["pool"]["xpt_target_address"] == encode_puzzle_hash(new_ph_3, "xpt")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "xpt")
